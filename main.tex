...
\begin{document}
	...
	\chapter{Our Construction}
	   ...
	   \label{sec:OurTransactionTypes}
        ...
        We realize the exchange of a product $x$ between seller $S$ and the buyer $B$ for price $p$ in protocol 1[]. For the exchange, $x$ is split up into $c$ chunks of data. We assume that $|x| = m*c $ where $m$ is representing the size of each chunk. At the beginning of the protocol, the Merkle tree of $x$ ($M_x$) is available to both parties as public knowledge. The Merkle tree leaves are the hashed chunks of $x$. Therefore, $B$ does not learn anything about $x$, besides its size. (Note that $M_x$ differs for the same $x$ if a different split $c$ is chosen.) Additionally, three (to the start time of the protocol) relative timelocks $t_{lock}, t_{refund}, t_{claim}$ are known to both parties. We require that $t_{lock} > t_{refund} > t_{claim}$. \\\\
        To encrypt $x$, S creates a new secret key $k$ and performs the Enoding routine (cf. ), which produces $ex$ (encrypted $x$) and $hash_k$ (the cryptographic hash of $k$). Then, $S$ lockes up $p$ coins via the Locking Transaction $TX_{lock}$ to eventually fund one of $i \in (1, ..., c)$ new Refund Transactions $TX_{{refund}_i}$ ($TX_{lock}$  is used as input transaction for each $TX_{{refund}_i}$. By creating $TX_{{refund}_i}$ with the correct values, $S$ makes sure that these transactions and, therefore, $TX_{lock}$ remain unspendable for $B$ (cf. []). For the next step $S$ sends all $TX_{{refund}_i}$, including $S$'s signature for the out script's, to $B$. Note that this implicitly shares $ex$ with $B$. Finally $S$ publishes $TX_{lock}$ to the ledger. \\\\
        $B$ receives all $TX_{{refund}_i}$ and extracts $ex'$ from the out scripts. We refer to the parsed encrypted data as $ex'$ because $B$ does not know if the encrypted data contains desired $x$ (results in $M_x$). $B$ then validates all refund transactions and checks for $TX_{lock}$ on the ledger. If no protocol violation is discovered, B proceeds. The actual data is already exchanged but without $k$ not accessible for B. To attain $k$, B creates the Key Exchange Transaction $TX_{exch}$ that locks $p$ coins through $hash_k$ and publishes it to the ledger. (Note that after $t_{claim}$ $B$ can unlock the funds. \\
        $S$ waits for $TX_{exch}$ to appear on the ledger. If this does not happen until $t_{claim}$, $S$ assumes that $B$ aborted the protocol and waits until $t_{lock}$ to claim $TX_{lock}$ and leave the protocol. This could potentially fail if $TX_{lock}$ is already spent by $B$ and implies that one of the refund transactions $TX_{{refund}_i}$ got published to the ledger. (cf. []) If this is the case, $S$ claims $TX_{{refund}_i}$. $S$ is able to do this, because $t_{refund}$ is passed at this point. If $TX_{exch}$ does appear on the ledger, $S$ claims $p$ coins through $TX_{exch}$ by providing $k$ s.t. $H(k) = hash_k$. The exchange is finished for $S$ and $S$ only needs to wait until $t_{lock}$ to claim the locked coins in $TX_{lock}$ back. \\\\
        $B$ waits for the spending of $TX_{exch}$ by $S$ to learn $k$. If this does not happen in time (until $t_{claim}$) $B$ assumes $S$ aborted the protocol, claims $TX_{exch}$ back and aborts. Otherwise, $B$ parses $k$ and decrypts $ex'$ through the Decoding routine and attains $x'$. If the expectation ($M_x$) is met, $x' = x$ and the exchange was successful. If this is not the case, the Decoding routine outputs the index $i$ of the (first) unexpected data chunk in $x'$. This enables $B$ to publish and claim $TX_{{refund}_i}$ by providing $k$. We will show this informally in the following section. 
