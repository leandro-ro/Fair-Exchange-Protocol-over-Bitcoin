...
\begin{document}
	...
	\chapter{Introduction}
        As shown in [], an exchange protocol is a particular subcase of multiparty computation. To archive security in multiparty computation protocols, several aspects are to be fulfilled, including fairness defined in [] as if one party receives output, then so do all. [10 in legal fariness] showed that it is impossible to archive this fairness when there is no honest majority among the protocol participants. If we transfer this knowledge into the context of a two-party exchange protocol, it is impossible to archive fairness at that point where there is one dishonest party involved. This is where approaches like a gradual release or the optimistic model (both also explained in []) come into play to archive alternative acceptable notions of fairness.
